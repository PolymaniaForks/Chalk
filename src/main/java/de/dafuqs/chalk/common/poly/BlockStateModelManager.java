package de.dafuqs.chalk.common.poly;
...
public class BlockStateModelManager {
    ...
    public static void addBlock(Identifier identifier, Block block, int color) {
        try {
            var path = FabricLoader.getInstance().getModContainer("chalk").get()
            ...
            if (modelDef.variants().isPresent()) {
                ...
                parseVariants(block, modelDef.variants().get(), list, color);
            ...
            if (modelDef.multipart().isPresent()) {
                ...
                parseMultipart(block, modelDef.multipart().get(), list, color);
    ...
    private static void parseMultipart(Block block, List<StateMultiPartDefinition> multiPartDefinition, ArrayList<Pair<List<BlockStatePredicate>, List<ModelData>>> list, int color) {
        for (var part : multiPartDefinition) {
            ...
            var modelData = parseBaseVariants(part.apply(), color);
    ...
    private static void parseVariants(Block block, Map<String, List<StateModelVariant>> modelDef, ArrayList<Pair<BlockStatePredicate, List<ModelData>>> list, int color) {
        start:
        for (var pair : modelDef.entrySet()) {
            ...
            var modelData = parseBaseVariants(pair.getValue(), color);
            list.add(new Pair<>(predicate, modelData));
        }
    }

    private static List<ModelData> parseBaseVariants(List<StateModelVariant> value, int color) {
        var modelData = new ArrayList<ModelData>();

        for (var v : value) {
            var stack = ItemDisplayElementUtil.getModelCopy(v.model());
